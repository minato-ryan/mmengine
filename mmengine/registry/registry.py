...
from mmengine.utils import get_object_from_string, is_seq_of, package_utils
...
class Registry:
        ...
        with DefaultScope.overwrite_default_scope(scope):
            ...
            if default_scope is not None:
                ...
                try:
                    ...
                    if scope in package_utils.OFFICAL_MODULE2PACKAGE:
                        print_log(
                            ...
                            f'`pip install {package_utils.OFFICAL_MODULE2PACKAGE[scope]}.',
                            logger='current',
                            level=logging.WARNING)
                    else:
                        print_log(
    ...
    def import_from_location(self) -> None:
        ...
        if not self._imported:
            ...
            if len(
                    self._locations
            ) == 0 and self.scope in package_utils.OFFICAL_MODULE2PACKAGE:
                print_log(
                ...
                try:
                    ...
                    if self.scope in package_utils.OFFICAL_MODULE2PACKAGE:
                        print_log(
                            f'{self.scope} is not installed and its '
                            'modules will not be registered. If you '
                            'want to use modules defined in '
                            f'{self.scope}, Please install {self.scope} by '
                            f'`pip install {package_utils.OFFICAL_MODULE2PACKAGE[self.scope]}.',
                            logger='current',
                            level=logging.WARNING)
                    else:
                        print_log(
